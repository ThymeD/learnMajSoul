...
- `RF-005` 项目交付管理模块（需求/缺陷/待办 + 进度可视化，支持三麻/四麻规则复核联动）（P1）
...
| 编号 | 需求主题 | 状态 | 负责人 | 备注 |
| --- | --- | --- | --- | --- |
| PM-DELIVERY-001 | 交付管理模块（项目迭代过程可视化管理） | 📝 待你确认规则细节 | PM/Coder | 上线前默认隐藏入口，迭代期可持续刷新 |

**PM-DELIVERY-001 需求优化稿（v0.1）**

- **目标定位**：提供一个“面向项目交付过程”的管理工作台，用统一交互方式管理需求、缺陷、待办与规则复核决策，支持持续迭代刷新。
- **核心能力**：
  1. 需求池管理：新增/拆分/状态流转（待分析→待实现→开发中→待验证→已完成）。
  2. 缺陷池管理：缺陷登记模板化、优先级、复现信息、验证证据追踪。
  3. 待办池管理：任务分解、负责人、截止时间、风险标记、阻塞关系。
  4. 规则复核联动：复用“规则复核面板”交互模型，覆盖雀魂三麻/四麻规则决策沉淀。
  5. 进度可视化：总览卡片（总数/进行中/阻塞/完成率）+ 趋势图 + 看板视图（按状态泳道）。
  6. 信息补充入口：每条记录支持补充说明、参考链接、附件证据入口（先支持文本/链接）。
- **上线与可见性约束**：
  - 该模块属于“交付辅助能力”，**生产上线前可配置隐藏**（路由守卫/配置开关控制菜单显示）。
  - 开发与迭代阶段默认可用，数据可持续累积并随迭代更新。
- **数据与扩展约束**：
  - 首期可本地持久化（localStorage）+ 结构可迁移（后续可切后端）。
  - 数据结构需区分 `mode=sanma/yonma/both`，便于规则与任务按玩法过滤。
- **验收标准（首期）**：
  1. 可在统一页面完成需求/缺陷/待办的增删改查与状态流转。
  2. 可视化面板实时反映当前进度（数量、完成率、阻塞项）。
  3. 每条记录具备“补充信息入口”，并可持久化。
  4. 可配置隐藏模块入口且不影响现有业务页面。
  5. 至少提供 1 组三麻、1 组四麻、1 组共通规则记录样例。

**用户已确认决策（2026-03-20）**

1. 规则基线：优先参考雀魂/日本立直麻将规则进行优化（含三麻/四麻）。
2. 边界策略：`手牌分析` 阶段不支持时序上下文役（抢杠/岭上/海底/河底/一发/两立直/天和地和等）。
3. 时序役实现阶段：放入后续高级功能（人机对局）再设计并实现。
...
| RF-005 | 交付管理模块：需求/缺陷/待办统一管理 + 进度可视化 + 可隐藏入口 | P1 | 待开始 | 复用规则复核交互模式，支持三麻/四麻/共通过滤 |
