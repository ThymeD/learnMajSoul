...
各agent提交绩效总结（举证机会）
    ↓
PM汇总绩效总结 + 客观评价（git记录、产出文件）
    ↓
PM初评（结合绩效总结 + 客观评价 + committer日常评价）
    ↓
用户审核初评结果（结合主观判断）
    ↓
用户确认评级
...
| 类型         | 定义                                | 来源      | 关闭条件                           |
| ------------ | ----------------------------------- | --------- | ---------------------------------- |
| **生产问题** | 用户登记的缺陷，功能不符合预期      | 用户发现  | 修复后必须复盘，用户确认后才能关闭 |
...
| 缺陷类型 | 引入责任人 | 说明 |
| -------- | ---------- | ---- |
| 生产问题 | 根据根因判定 | 可能是需求、方案、实现、审核等阶段 |
| 测试问题 | 实现者 | 通常是Coder |

### 责任人陈述机制

引入责任人有权进行陈述，确保责任判定的公正性。
...
↓
选择：
├── 承认引入 → 提交陈述报告（承认错误、分析原因、改进计划）
└── 进行申述 → 提交申述报告（说明理由、提供证据）
↓
PM审核陈述/申述
↓
确认责任人归属
...
在绩效考评中，缺陷引入数作为重要参考维度：

| 维度 | 权重 | 说明 |
| ---- | ---- | ---- |
| 缺陷引入数 | 20% | 引入缺陷越少评分越高 |
| 缺陷发现贡献 | 10% | 发现问题贡献越大评分越高 |

### 劳动态度考核

**核心理念**：虽然结果重要，但态度同样重要。努力过了即使结果不好也可以拿优秀。

| 维度 | 权重 | 说明 |
| ---- | ---- | ---- |
| 努力程度 | 10% | 是否全力以赴 |
| 积极程度 | 10% | 是否主动发现问题和提出改进 |
| 协作程度 | 10% | 是否积极配合、乐于助人 |

**评价原则**：
- 短期：结果好+态度好 = 优秀；结果差+态度好 = 可以接受
- 长期：不存在态度好但结果差的agent，态度好最终会体现到结果上
- 态度差+结果好 = 不可持续，需要改进
- 态度差+结果差 = 淘汰
...
各agent识别自身不足
↓
更新自身描述文件（.opencode/agents/{agent}.md）
↓
提交git
↓
PM汇总优化内容
...
| -------------------- | ---------------------------- |
| PM                   | 协调流程、缺陷管理、时间记录 |
| Requirements Manager | 需求审核清单、边界条件识别   |
| Architect            | 方案设计清单、技术选型依据   |
| Coder                | 自测清单、编码规范、边界处理 |
| Committer            | 审核清单、功能验证环节       |
| Automation Tester    | 测试用例设计、边界条件覆盖   |
| UI Tester            | 测试用例、交互细节识别       |
...
| **ui-designer**          | 界面设计                 | design/_.html, design/_.md |
| **coder**                | 代码实现                 | src/\*\*                   |
| **committer**            | 代码审核 + 合入develop   | 审核意见                   |
| **automation-tester**    | 自动化测试               | tests/\*_/_.test.ts        |
| **ui-tester**            | UI测试（人工复核）       | tests/ui/\*\*              |
| **docs-writer**          | 文档编写                 | docs/\*_/_.md              |
...
3. **完成后**：
   - 确保代码可运行
   - **报告完成时间（格式：YYYY-MM-DD HH:MM:SS），用于协作文件时间戳记录**
   - 向用户汇报完成情况
   - 用户关闭当前窗口
4. **等待交接**：
   - 用户切换到PM窗口，告知完成情况
...
| 感知方式     | 说明                                                 |
| ------------ | ---------------------------------------------------- |
| **用户告知** | 用户切换到PM窗口时告知某个任务已完成，并提供完成时间 |
| **用户指令** | 用户告知PM"Agent A 完成了"                           |

**注**：完成时间格式为 `YYYY-MM-DD HH:MM:SS`，由agent在任务完成时报告，PM据此填写协作文件时间戳。
